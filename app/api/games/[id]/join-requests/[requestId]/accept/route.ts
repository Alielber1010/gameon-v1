...
// POST /api/games/[id]/join-requests/[requestId]/accept - Accept a join request
export async function POST(
  ...
  try {
    ...
    // Check if user is already a registered player (prevent duplicates)
    const isAlreadyPlayer = game.registeredPlayers.some(
      (player: any) => player.userId.toString() === joinRequest.userId.toString()
    );

    if (isAlreadyPlayer) {
      // Remove the duplicate join request
      game.joinRequests.pull(requestId);
      await game.save();
      return NextResponse.json(
        { success: false, error: 'User is already a registered player' },
        { status: 400 }
      );
    }
    ...
    const playerData = {
      ...
      age: joinRequest.age,
      skillLevel: joinRequest.skillLevel,
      image: joinRequest.image,
      whatsApp: joinRequest.whatsApp,
      joinedAt: new Date(),
    };
    ...
    // Get game and user details for notification
    const gameTitle = game.title;
    const requesterUser = await User.findById(joinRequest.userId);
    ...
    if (requesterUser) {
      const notification = new Notification({
        ...
        message: `Your request to join "${gameTitle}" has been accepted!`,
        gameId: game._id,
        read: false,
      });
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to accept join request' },
      { status: 500 }
    );
  }
}
